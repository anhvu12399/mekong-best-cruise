...
export function FullMekongMap({ activeDay, onDayChange }: Props) {
  ...
  useEffect(() => {
    const handleMessage = (e: MessageEvent) => {
      if (e.data?.type === 'DAY_CLICKED') {
        let targetDay = 0
        if (e.data.day === 0) targetDay = 0
        if (e.data.day === 1) targetDay = 1
        if (e.data.day === 2) targetDay = 2
        if (e.data.day === 3) targetDay = 4 // Luang Prabang starts Day 5
        if (e.data.day === 4) targetDay = 6 // Pakbeng starts Day 7
        if (e.data.day === 5) targetDay = 7 // Houay Xai starts Day 8
        if (e.data.day === 6) targetDay = 8 // Golden Triangle starts Day 9
  ...
  const htmlContent = `
    ...
    <body>
      <div id="map"></div>
      <script>
        const STOPS = ${JSON.stringify(FULL_MEKONG_STOPS)};
        ...
        // South Vietnam Route (HCMC -> Ben Tre -> Can Tho)
        L.polyline(lineCoords.slice(0, 3), {
          color: '#8B4A2A', weight: 1.5, opacity: 0.8
        }).addTo(map);

        // Flight (Can Tho -> Luang Prabang)
        L.polyline([lineCoords[2], lineCoords[3]], {
          color: '#4a4a4a', weight: 1.2, dashArray: '4, 8', opacity: 0.6
        }).addTo(map);

        // Laos Route (Luang Prabang -> Pakbeng -> Houay Xai -> Golden Triangle)
        L.polyline(lineCoords.slice(3, 7), {
          color: '#8B4A2A', weight: 1.5, opacity: 0.8
        }).addTo(map);
        ...
        function getActiveMarkerIndex(day) {
          ...
          if (day === 1) return 1; // Ben Tre
          if (day === 2 || day === 3) return 2; // Can Tho
          if (day === 4 || day === 5) return 3; // Luang Prabang
          if (day === 6) return 4; // Pakbeng
          if (day === 7) return 5; // Houay Xai
          if (day === 8 || day === 9) return 6; // Golden Triangle
          return 0;
        }
  ...
  const getActiveLabel = () => {
    if (activeDay === 0) return "Ho Chi Minh City, Vietnam"
    if (activeDay === 1) return "Bến Tre (Mekong Delta), Vietnam"
    if (activeDay === 2 || activeDay === 3) return "Cần Thơ (Mekong Delta), Vietnam"
    if (activeDay === 4 || activeDay === 5) return "Luang Prabang, Laos"
    if (activeDay === 6) return "Pakbeng, Laos"
    if (activeDay === 7) return "Houay Xai, Laos"
    if (activeDay === 8 || activeDay === 9) return "Golden Triangle"
    return "Mekong River"
  }
  ...
  return (
    <div className="relative w-full h-full bg-[#e8e4db]">
      <iframe
        ...
        title="Full Mekong Story Route Map"
